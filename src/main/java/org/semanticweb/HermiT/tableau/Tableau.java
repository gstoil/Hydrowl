...
/**
 * This class overrides the standard Tableau class of HermiT. It includes all functionality 
 * of the standard HermiT Tableau class but it additionally offers the ability for a sort 
 * of incremental reasoning. More precisely, after applying the hypertableau calculus to an 
 * ontology this class adds a dummy branching point at the end. Then, it offers the method 
 * isSatisfiableContinue which backtracks to the previously added branching point and continues
 * the calculus from there on. This functionality can be used, e.g., for saturating a large input
 * ontology O with the hypertableau rules and then continue each call from that point on without
 * the need to reapply all rules on all axioms of O. 
 * 
 * This class is used in combination with edu.ntua.image.reasoning.ReasonerExt which extends the 
 * HermiT Reasoner class to properly use this Tableau class.      
 */
public final class Tableau implements Serializable {
    ...
    protected DLOntology m_secondAdditionalDLOntology;
    ...
    protected HyperresolutionManager m_secondAdditionalHyperresolutionManager;
    ...
    protected Stack<BranchingPoint> m_dummyBranchingPoints;
    ...
    public Tableau(InterruptFlag interruptFlag,TableauMonitor tableauMonitor,ExistentialExpansionStrategy existentialsExpansionStrategy,boolean useDisjunctionLearning,DLOntology permanentDLOntology,DLOntology additionalDLOntology,Map<String,Object> parameters) {
        ...
        try {
            ...
            m_secondAdditionalHyperresolutionManager=null;
            ...
            m_dummyBranchingPoints=new Stack<BranchingPoint>();
        }
        finally {
    ...
    public void clear() {
        ...
        if (m_secondAdditionalHyperresolutionManager!=null)
        	m_secondAdditionalHyperresolutionManager.clear();
...
//        		(additionalDLOntology.hasNominals() && !hasNominals) || 
        		(!additionalDLOntology.isHorn() && isHorn) || 
    ...
    public void setSecondAdditionalDLOntology(DLOntology additionalDLOntology) {
        if (!supportsAdditionalDLOntology(additionalDLOntology))
            throw new IllegalArgumentException("Additional DL-ontology contains features that are incompatible with this tableau.");
        m_secondAdditionalDLOntology=additionalDLOntology;
        m_secondAdditionalHyperresolutionManager=new HyperresolutionManager(this,m_secondAdditionalDLOntology.getDLClauses());
        m_existentialExpansionStrategy.additionalDLOntologySet(m_secondAdditionalDLOntology);
        m_datatypeManager.additionalDLOntologySet(m_secondAdditionalDLOntology);
        updateFlagsDependentOnAdditionalOntology();
    }
    ...
    public void clearSecondAdditionalDLOntology() {
        m_additionalDLOntology=null;
        m_secondAdditionalDLOntology=null;
        m_secondAdditionalHyperresolutionManager=null;
        m_existentialExpansionStrategy.additionalDLOntologyCleared();
        m_datatypeManager.additionalDLOntologyCleared();
        updateFlagsDependentOnAdditionalOntology();
    ...
    public boolean isSatisfiable(boolean loadPermanentABox,boolean loadAdditionalABox,Set<Atom> perTestPositiveFactsNoDependency,Set<Atom> perTestNegativeFactsNoDependency,Set<Atom> perTestPositiveFactsDummyDependency,Set<Atom> perTestNegativeFactsDummyDependency,Map<Term,Node> termsToNodes,Map<Individual,Node> nodesForIndividuals,ReasoningTaskDescription reasoningTaskDescription) {
        ...
        BranchingPoint branchingPoint=new BranchingPoint(this);
        pushBranchingPoint(branchingPoint);
//        m_dummyBranchingPoint=branchingPoint.getLevel();
        m_dummyBranchingPoints.push(branchingPoint);
//        DependencySet dependencySet=m_dependencySetFactory.addBranchingPoint(m_dependencySetFactory.emptySet(),branchingPoint.getLevel());
//        Individual dummyIndividual=Individual.createAnonymous("dummy-indiv");
//        loadPositiveFact(termsToNodes,Atom.create(AtomicConcept.create("internal:DummyConcept"),dummyIndividual),dependencySet);
        if (m_tableauMonitor!=null)
            m_tableauMonitor.isSatisfiableFinished(reasoningTaskDescription,result);
        return result;
    }
    public boolean isSatisfiableContinue(boolean loadAdditionalABox,Set<Atom> perTestPositiveFactsNoDependency,Set<Atom> perTestNegativeFactsNoDependency,Set<Atom> perTestPositiveFactsDummyDependency,Set<Atom> perTestNegativeFactsDummyDependency,Map<Individual,Node> nodesForIndividuals,ReasoningTaskDescription reasoningTaskDescription) {
        boolean loadPermanentABox=m_permanentDLOntology.hasNominals() || (m_additionalDLOntology!=null && m_additionalDLOntology.hasNominals());
        return isSatisfiableContinue(loadPermanentABox,loadAdditionalABox,perTestPositiveFactsNoDependency,perTestNegativeFactsNoDependency,perTestPositiveFactsDummyDependency,perTestNegativeFactsDummyDependency,new HashMap<Term,Node>(),nodesForIndividuals,reasoningTaskDescription);
    }
    public boolean isSatisfiableContinue(boolean loadPermanentABox,boolean loadAdditionalABox,Set<Atom> perTestPositiveFactsNoDependency,Set<Atom> perTestNegativeFactsNoDependency,Set<Atom> perTestPositiveFactsDummyDependency,Set<Atom> perTestNegativeFactsDummyDependency,Map<Term,Node> termsToNodes,Map<Individual,Node> nodesForIndividuals,ReasoningTaskDescription reasoningTaskDescription) {
        if (m_tableauMonitor!=null)
            m_tableauMonitor.isSatisfiableStarted(reasoningTaskDescription);
        backtrackTo(m_dummyBranchingPoints.peek().getLevel());
        if (loadAdditionalABox && m_additionalDLOntology!=null) {
            for (Atom atom : m_additionalDLOntology.getPositiveFacts())
                loadPositiveFact(termsToNodes,atom,m_dependencySetFactory.emptySet());
            for (Atom atom : m_additionalDLOntology.getNegativeFacts())
                loadNegativeFact(termsToNodes,atom,m_dependencySetFactory.emptySet());
            if (m_secondAdditionalDLOntology!=null) {
                for (Atom atom : m_secondAdditionalDLOntology.getPositiveFacts())
                    loadPositiveFact(termsToNodes,atom,m_dependencySetFactory.emptySet());
                for (Atom atom : m_secondAdditionalDLOntology.getNegativeFacts())
                    loadNegativeFact(termsToNodes,atom,m_dependencySetFactory.emptySet());
            }
        }
        if (perTestPositiveFactsNoDependency!=null && !perTestPositiveFactsNoDependency.isEmpty())
...
//        }
    ...
    protected boolean doIteration() {
        if (!m_extensionManager.containsClash()) {
            ...
            while (m_extensionManager.propagateDeltaNew() && !m_extensionManager.containsClash()) {
                ...
                if (m_secondAdditionalHyperresolutionManager!=null && !m_extensionManager.containsClash())
                	m_secondAdditionalHyperresolutionManager.applyDLClauses();
	...
	public void backtrackToAndRemoveLastDummyBracnhingPoint() {
		backtrackTo(m_dummyBranchingPoints.pop().getLevel());
	}
	public void incrementModel(ReasoningTaskDescription reasoningTaskDescription) {
        if (m_tableauMonitor!=null)
            m_tableauMonitor.isSatisfiableStarted(reasoningTaskDescription);
        if (m_firstTableauNode==null)
            createNewNINode(m_dependencySetFactory.emptySet());
