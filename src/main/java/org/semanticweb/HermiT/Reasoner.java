...
//            		}
        		...
        		if (add) {
                	OWLAxiom[] toAddAsArray = new OWLAxiom[toAdd.size()];
                	toAdd.toArray(toAddAsArray);
                	m_tableau=getTableau2(toAddAsArray);
                    m_tableau.incrementModel(ReasoningTaskDescription.isABoxSatisfiable());
        		}
        		else {
        			m_tableau.backtrackToAndRemoveLastDummyBracnhingPoint();
...
//            }
    ...
    public void initialiseClassInstanceManager() {
    ...
    public boolean isConsistentContinue() {
        m_isConsistent=m_tableau.isSatisfiableContinue(true,null,null,null,null,null,ReasoningTaskDescription.isABoxSatisfiable());
        return m_isConsistent;
    }
...
//                for (Atom atom : deltaDLOntology.getPositiveFacts()) {
//                    System.out.println("asdasdas: " + atom);
//                }
                return m_tableau;
            }
            else
     ...
     * gstoil additions
     */
    public NodeSet<OWLNamedIndividual> getInstances(OWLClass classExpression,boolean direct,Set<String> possibleIndividuals,Set<String> knownIndividuals) {
        if (m_dlOntology.getAllIndividuals().size()>0) {
            checkPreConditions(classExpression);
            if (!m_isConsistent) {
                Node<OWLNamedIndividual> node=new OWLNamedIndividualNode(getAllNamedIndividuals());
            ...
            initialiseClassInstanceManager();
            Set<Individual> result=new HashSet<Individual>();
            result=m_instanceManager.getInstances(H(classExpression),direct,possibleIndividuals,knownIndividuals);
            return sortBySameAsIfNecessary(result);
        }
        else
            return new OWLNamedIndividualNodeSet(new HashSet<Node<OWLNamedIndividual>>());
